...
- [x] T5.3 Criar ferramentas de ajuste rapido
  Notas: `RPG-32` abriu um primeiro bloco de `Ajuste rapido` no cockpit ao vivo, derivado de `livePressure` + encontro preparado, com sugestoes curtas de aliviar, segurar ou escalar a tensao sem automatizar a mesa. `RPG-235` refinou esse ajuste para reagir tambem a estresse de PM/SAN. `RPG-236` removeu duplicacao estrutural entre os blocos tatico e narrativo (`Sinais ao vivo` e `Ajuste rapido`) com componentes reutilizaveis, mantendo o comportamento. `RPG-239` qualificou as acoes do ajuste para diferenciar explicitamente cenarios de `resourceStressed/resourceCritical` (PM/SAN) com orientacao mais acionavel de respiro e preservacao de recurso. `RPG-242` priorizou recomendacao de estabilizacao/recuperacao quando ja ha personagem caido, inclusive em `rising`. `RPG-243` introduziu postura conservadora em `stable` quando o encontro preparado ja e `deadly/punitivo`, evitando escalada sugerida indevida. `RPG-245` extraiu limiares de pressao/recursos para constantes calibraveis no motor ao vivo.
